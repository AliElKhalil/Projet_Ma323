...
def matrice_ec(N,c,d):
    A = (1-2*c)*np.eye(N)+(c+d/2)*np.eye(N,N,-1)+(c-d/2)*np.eye(N,N,1)
    return(A)

#Construction matrice implicite décentré amont
def matrice_ic(N,c,d):
    A = (1+2*c)*np.eye(N)+(-d/2-c)*np.eye(N,N,-1)+(d/2-c)*np.eye(N,N,1)
    return(A)
...
#résolution
def SolEC(h,N,tau,c,d):
    ntfinal=int(Tmax/tau)+1
    A_EC = matrice_ec(N,c,d)
    UT=np.zeros((ntfinal,N))
    T=np.arange(ntfinal)*tau
    X=construitX(N)
    a=int(input("Voulez-vous réaliser les tests pour U0 question 8 ou U0 question 9 ?"))
    if (a==8):
        U=U0(X[0:N])
    else:
        U=U02(X[0:N])
...
def SolIC(h,N,tau,c,d):
    ntfinal=int(Tmax/tau)+1
    A_IC = matrice_ic(N,c,d)
    UT=np.zeros((ntfinal,N))
    T=np.arange(ntfinal)*tau
    X=construitX(N)
    U=U0(X[0:N])
...
Solveurs=[SolEC,SolIC] #Initialisation du veteur solveurs reprenant toutes les méthodes que nous allons utiliser
Nomssolveurs=['Méthode explicite centrée','Méthode implicite centrée'] #Initialisation du vecetur reprenan toute sles chaines de caractères de chaque méthode
